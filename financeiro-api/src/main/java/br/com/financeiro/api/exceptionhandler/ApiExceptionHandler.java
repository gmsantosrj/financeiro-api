...
import br.com.financeiro.domain.exception.EntidadeNaoEncontradaException;
import br.com.financeiro.domain.exception.ValidationException;
import br.com.financeiro.infrastructure.service.arquivo.ArquivoContaException;
...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<?> handleUnauthorizedException(HttpClientErrorException.Unauthorized ex, WebRequest request) {
        HttpStatus status = HttpStatus.UNAUTHORIZED;
        ProblemType problemType = ProblemType.ACESSO_NEGADO;
        String detail = ex.getMessage();
        
        Problem problem = createProblemBuilder(status, problemType, detail).userMessage(detail)
				.userMessage("Você não está autorizado a acessar este recurso. Por favor, verifique suas credenciais ou entre em contato com o administrador do sistema.").build();
	...
	private Problem.ProblemBuilder createProblemBuilder(HttpStatusCode status, ProblemType problemType, String detail) {
		...
		return Problem.builder()
				.timestamp(OffsetDateTime.now())
				.status(status.value()).type(problemType.getUri())
	...
	@ExceptionHandler(ArquivoContaException.class)
	protected ResponseEntity<?> handleArquivoMovimentacaoException(ArquivoContaException ex, WebRequest request) {
	...
	private ResponseEntity<Object> handleValidationException(ValidationException ex, WebRequest request) {
		...
		List<Problem.Object> problemObjects = ex.getErros().stream().map(objectError -> {
